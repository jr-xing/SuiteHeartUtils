from utils.contour_utils import rescale_contours_seq, generate_binary_masks_from_contour_seq
...
def get_SuiteHeart_dict_myocardium_rescaled_contours_and_masks(SuiteHeart_datum_dict, target_image_shape=(128,128), centering=False, check_slice_indices=None, rescale_method='pixel_spacing', rescale_ratio=0.6, force_keep_slice_dim=False):
    """
    ...
    if mat_has_single_slice_flag == True and not force_keep_slice_dim:
...
import numpy as np
def get_SuiteHeart_dict_resized_and_cropped_images(SuiteHeart_datum_dict, target_image_shape=(128,128), rescale_method='pixel_spacing', rescale_ratio=0.6, force_keep_slice_dim=False):
    """
...
def get_SuiteHeart_dict_cropped_images(SuiteHeart_datum_dict, target_image_shape=(128,128), force_keep_slice_dim=False):
...
def get_SuiteHeart_dict_myocardium_masks(SuiteHeart_datum_dict, ori_image_shape=(128,128), target_image_shape=(128,128), centering=True, check_slice_indices = None, force_keep_slice_dim=False):
    """
    Get the myocardium contours and masks from a SuiteHeart dictionary.
